...
class pointscandriver:
    ...
    def __init__(self):
        try:
            ...
            self.tt.setTriggerLevel(self.SPD_CH, 0.075)
    ...
    def set_trigger_level(self, channel, level):
        self.tt.setTriggerLevel(channel, level)
...
if __name__ == "__main__":
    ...
    for i in range(50):
        new_level = 2.5
        print(f"Setting trigger level to {new_level}")
        ps.set_trigger_level(ps.SPD_CH, new_level)
        img = ps.fullscan()
        print(img.shape)
        print("sum of img: ", img.sum())
